...
function Import(contractingproessid, database, numberStage, registerid) {
    ...
    this.importParties = async function(parties) {
        let schema = {
            ...
            identifier: {
                ...
                juridicalPersonhood: (juridicalPersonhood, result) => result.naturalPerson = juridicalPersonhood === 'naturalPerson',
                givenName: 'givenname',
            ...
            contactPoint: {
                ...
                availableLanguage: (availableLanguage, result) => result.contactpoint_language = availableLanguage ? availableLanguage.join(',') : undefined
            },
            position: 'position'
        };
        ...
        await processRecords('parties',['contractingprocess_id', 'partyid'], schema, {contractingprocess_id: cpid}, parties, async party => {
            ...
            if(party.additionalContactPoints) {
                await processRecords('additionalcontactpoints', ['party_id'], 
                {
                    ...
                    faxNumber: 'faxnumber',
                    url: 'url',
                    availableLanguage: (availableLanguage, result) => result.language = availableLanguage ? availableLanguage.join(',') : undefined
                }, 
                {
                    party_id: party.id
                }, 
    ...
    this.importContracts = async function(contracts, contractid) {
        ...
        await processRecords('contract',identifiers , schema, annexes, contracts, async contract => {
            ...
            await processRecords('guarantees', ['contractingprocess_id', 'contract_id' ,'guarantee_id'], {
                ...
                date: 'date',
                guaranteedObligations: 'guaranteedobligations',
                value: {
                    amount: 'value',
                    currency: 'currency'
                },
    ...
    let importClarificationMeetings = async function(clarificationMeetings){
        ...
        await processRecords('clarificationmeeting', ['contractingprocess_id', 'clarificationmeetingid'], schema, { contractingprocess_id: cpid }, clarificationMeetings, async clarification => {
            ...
            await processRecords('clarificationmeetingactor', ['clarificationmeeting_id', 'parties_id'], schemaActor, { clarificationmeeting_id: clarification.id, attender: true }, clarification.clarificationMeetingAttendees);
            await processRecords('clarificationmeetingactor', ['clarificationmeeting_id', 'parties_id'], schemaActor, { clarificationmeeting_id: clarification.id, official: true }, clarification.clarificationMeetingOfficials);
        });
    }
