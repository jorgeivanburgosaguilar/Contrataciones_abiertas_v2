...
let build = (release, recordsPnt, position, extras) => {
    ...
    release.contracts.map(contract => {
        ...
        fn.add(334314, start.toISOString(), date => fn.dateFormat(date), true, 'Fecha de inicio del periodo que se informa');
        fn.add(334315, end.toISOString(), date => fn.dateFormat(date), true, 'Fecha de término del periodo que se informa');
        ...
        values = values.concat(fn.findValues(`tender.documents[?(@.documentType==="biddingDocuments")].url`));
        values = values.concat(fn.findValues(`tender.documents[?(@.documentType==="technicalSpecifications")].url`));
        ...
        fn.addTable(334306, 'parties[?(@.roles.indexOf("issuingSupplier")!==-1)]', (party, results) => {
        ...
        if (juntas) {
            juntas.map(junta => {
                ...
                fn.addTable(334307, 'clarificationMeetingAttendees', (party, results) => {
                ...
                fn.addTable(334308, 'clarificationMeetingOfficials', (party, results) => {
                    ...
                    fn.addInternalField(43335, `identifier.position`, party, results);
                    fn.addInternalField(43336, `parties[?(@.id==="${party.id}")].id`, release, results);
                }, junta);
            });
        }

        // en el mapeo dise que deberian ser clarificationMeeting
        ...
        values = fn.findValue('tender.awardCriteria');
        switch (values) {
            case 'priceOnly':
                fn.add(334326, 'Resultó la proposición solvente económicamente más baja.');
                break;
            case 'ratedCriteria':
                fn.add(334326, 'Su proposición cumplió con los requisitos, normativos, tecnicos y economicos, y obtuvo el mejor puntaje de conformidad con lo  previsto en la convocatoria.');
                break;
        }

        fn.add(334285, extras.usuario);
        fn.add(334284, extras.usuario);

        fn.add(334286, extras.usuario);

        fn.addField(334281, 'id', contract);
        fn.addField(334290, 'dateSigned', contract, date => fn.dateFormat(date));
        ...
        values = fn.findValues('documents[?(@.documentType==="suspensionStatement")].url', contract);
        ...
        fn.add(334313, 0);
        fn.add(334333, 'Recursos fiscales');
        fn.add(334276, 'Respecto al tipo de fondo de participación o aportación respectiva, los recursos de este Instituto se encuentran en el ramo 44, para el cual no se consideran fondos de participación o aportación');
        ...
        values = fn.findValues('items[?(@.description)].description', contract);
        fn.add(334319, values.join(','));
        fn.addField(334329, 'planning.documents[?(@.documentType==="environmentalImpact")].url');
        // no aplica
        //fn.addFields(334330, 'tender.milestones[?(@.type=="publicNotices")].description');
        ...
        fn.addField(334298, 'implementation.documents[?(@.documentType=="physicalProcessReport")].url', contract);
        fn.addField(334303, 'implementation.documents[?(@.documentType=="financialProgressReport")].url', contract);
        // es de esta etapa?
        // no se envia
        fn.addField(334299, 'documents[?(@.documentType=="physicalReception")].url', contract);
        fn.addField(334300, 'implementation.documents[?(@.documentType=="finalPayment")].url', contract);

        fn.add(334321, extras.usuario);
