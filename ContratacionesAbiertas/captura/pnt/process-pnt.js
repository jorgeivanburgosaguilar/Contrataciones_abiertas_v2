...
// email de la unidad 
const EMAIL = process.env.PNT_EMAIL || 'correo@institucional';
...
let sendRegisters = async (cps, throwError) => {
    ...
    let processFormat = async (cp, release, format, position, date, user) => {
        ...
        let extras =  (await edca_db.oneOrNone('select year as ejercicio from budgetclassifications where budgetbreakdown_id in (select id from budgetbreakdown where contractingprocess_id = $1) limit 1',[cp])) || {};
        extras.fechaActualizacion = (new Date()).toISOString();
        extras.fechaValidacion = date;
        extras.usuario = user;
