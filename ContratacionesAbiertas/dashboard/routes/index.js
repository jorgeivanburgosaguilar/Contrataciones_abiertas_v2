...
router.post('/pagination', function (req, res) {
    ...
    edca_db.task(function (t) {
        ...
        query1 += " order by $6~" + (req.body.orderby === 'value_amount' || req.body.orderby === 'datesigned' ? " desc" : "");
                ...
                t.task(function (t2) {
                    return t2.oneOrNone(planningQuery, [data.id]).then(function (e) {
                        return t2.batch([
                            ...
                            t2.task(function (t3) {
                                return t3.map("select quotes.id, quotes.quotes_id, quotes.description, parties.name as supplier, to_char(period_enddate, 'DD/MM/YYYY') period_enddate,  to_char(period_startdate, 'DD/MM/YYYY') period_startdate, to_char(date, 'DD/MM/YYYY') date, value from quotes inner join requestforquotes on requestforquotes.id = quotes.requestforquotes_id inner join parties on parties.id = quotes.issuingsupplier_id where requestforquotes.contractingprocess_id = $1", [data.id], function (e2) { // cotizacion planeacion
                                    return t3.batch([
                ...
                t.task(function (t2) {
                    return t2.oneOrNone(tenderQuery, [data.id]).then(function (e) {
                        return t2.batch([
                            ...
                            t2.manyOrNone("select parties.name from parties inner join roles on roles.parties_id = parties.id where parties.contractingprocess_id = $1 and roles.tenderer = true", [data.id]), // licitantes licitacion
